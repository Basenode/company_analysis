...
from industry_profile import get_industry_profile
...
class TushareClient:
    ...
    def get_basic_info(self, ts_code: str) -> str:
        ...
        industry_profile = get_industry_profile(ts_code, raw_industry)
        self._store["industry_profile"] = {
            "raw_industry": industry_profile.raw_industry,
            "industry_type": industry_profile.industry_type,
            "core_drivers": list(industry_profile.core_drivers),
            "forbidden_valuation_methods": list(industry_profile.forbidden_valuation_methods),
            "recommended_factors": list(industry_profile.recommended_factors),
            "notes": industry_profile.notes,
            "mapping_source": industry_profile.mapping_source,
            "needs_manual_confirm": industry_profile.needs_manual_confirm,
        }
        ...
        info_table = format_table(
            ...
            [
                ...
                ["行业", raw_industry],
                ["适配行业类型", industry_profile.industry_type],
                ["核心驱动因子", " / ".join(industry_profile.core_drivers) if industry_profile.core_drivers else "—"],
                ["禁用估值方法", " / ".join(industry_profile.forbidden_valuation_methods) if industry_profile.forbidden_valuation_methods else "—"],
                ["建议执行因子", " / ".join(industry_profile.recommended_factors) if industry_profile.recommended_factors else "—"],
                ["行业映射来源", industry_profile.mapping_source],
                ["行业需人工确认", "是" if industry_profile.needs_manual_confirm else "否"],
                ["地区", str(row.get("area", ""))],
    ...
    def _get_basic_info_hk(self, ts_code: str) -> str:
        ...
        row = basic.iloc[0]
        industry_profile = get_industry_profile(ts_code, "")
        self._store["industry_profile"] = {
            "raw_industry": industry_profile.raw_industry,
            "industry_type": industry_profile.industry_type,
            "core_drivers": list(industry_profile.core_drivers),
            ...
            for fy in fiscal_years:
                ...
                for _, r in fy_df.iterrows():
                    ...
                    rows.append([
                        ...
                        str(r.get("ex_date", "—")),
                        format_number(total_div, divider=1, decimals=2),
    ...
    def get_fina_indicators(self, ts_code: str) -> str:
        ...
        roe_disclosed_list: list[float | None] = []
    ...
    def get_audit(self, ts_code: str) -> str:
        """Section 7.1: Audit opinion from fina_audit."""
        lines = [format_header(2, "7.1 审计意见"), ""]
        ...
        for _, r in df.iterrows():
            ...
            fees_str = f"{fees / 10000:.1f}" if fees is not None and fees == fees else "—"
            rows.append([year, opinion, agency, fees_str])
        lines.append(format_table(headers, rows, alignments=["l", "l", "l", "r"]))
        return "\n".join(lines)
...
| 4 | 创始人/关键人物 | "{公司名} 创始人 背景" |"""),
    ...
    def get_repurchase(self, ts_code: str) -> str:
        ...
        # Calculate years span correctly - use actual 3-year period
        years_span = 3  # Always use 3 years for annual average
    ...
    def get_governance_relief(self, ts_code: str) -> str:
        """Section 18: Governance relief panel."""
        lines = [format_header(2, "18. 治理缓释面板"), ""]

        # Get pledge data
        pledge_ratio = "—"
        try:
            df = self._safe_call("pledge_stat", ts_code=ts_code, fields="ts_code,end_date,pledge_ratio")
            if not df.empty:
                latest = df.sort_values("end_date", ascending=False).iloc[0]
                pledge_ratio = f"{latest.get('pledge_ratio', 0):.2f}%"
        except Exception:
            pass

        # Get top 10 holders for controlling shareholder
        controlling_shareholder = "—"
        controlling_ratio = "—"
        try:
            df = self._safe_call("top10_holders", ts_code=ts_code, fields="ts_code,end_date,holder_name,hold_ratio,is_controll")
            if not df.empty:
                # Filter for controlling shareholder
                control_df = df[df.get("is_controll", 0) == 1]
                if not control_df.empty:
                    latest = control_df.sort_values("end_date", ascending=False).iloc[0]
                    controlling_shareholder = str(latest.get("holder_name", "—"))
                    controlling_ratio = f"{latest.get('hold_ratio', 0):.2f}%"
                else:
                    # Fallback to largest holder
                    latest = df.sort_values(["end_date", "hold_ratio"], ascending=False).iloc[0]
                    controlling_shareholder = str(latest.get("holder_name", "—"))
                    controlling_ratio = f"{latest.get('hold_ratio', 0):.2f}%"
        except Exception:
            pass
        ...
        auditor_changes = "—"
        try:
            df = self._safe_call("fina_audit", ts_code=ts_code, fields="ts_code,end_date,audit_result,audit_agency")
            if not df.empty:
                df = df.sort_values("end_date", ascending=False)
                latest = df.iloc[0]
                audit_opinion = str(latest.get("audit_result", "—"))
                # Count auditor changes in last 5 years
                agencies = df.head(5)["audit_agency"].dropna().unique()
                auditor_changes = str(len(agencies)) if len(agencies) > 0 else "—"
        except Exception:
            pass
        ...
        repurchase_amount = "—"
        try:
            df = self._safe_call("repurchase", ts_code=ts_code, fields="ts_code,ann_date,amount")
            if not df.empty:
                # Sum repurchase amount in last 2 years
                two_years_ago = (pd.Timestamp.now() - pd.DateOffset(years=2)).strftime("%Y%m%d")
                recent = df[df["ann_date"] >= two_years_ago]
                if not recent.empty:
                    total_amount = recent["amount"].sum()
                    repurchase_amount = format_number(total_amount, divider=1, decimals=2)
        except Exception:
            pass

        rows = [
            ["质押率", pledge_ratio],
            ["实控人", controlling_shareholder],
            ["实控人持股比例", controlling_ratio],
            ["最新审计意见", audit_opinion],
            ["近5年审计师变更次数", auditor_changes],
            ["近2年回购金额 (万元)", repurchase_amount],
        ]

        lines.append(format_table(["项目", "数值"], rows, alignments=["l", "r"]))
        return "\n".join(lines)

    def get_dynamic_debt(self, ts_code: str) -> str:
        """Section 19: Dynamic debt repayment panel."""
        lines = [format_header(2, "19. 动态偿债面板"), ""]

        # Get cash flow data for OCF
        ocf = "—"
        try:
            df = self._safe_call("cashflow", ts_code=ts_code, report_type="1", fields="ts_code,end_date,n_cashflow_act")
            if not df.empty:
                latest = df.sort_values("end_date", ascending=False).iloc[0]
                ocf = format_number(latest.get("n_cashflow_act", 0))
        except Exception:
            pass

        # Get balance sheet data for short-term debt
        short_debt = "—"
        try:
            df = self._safe_call("balancesheet", ts_code=ts_code, report_type="1", fields="ts_code,end_date,st_borr,non_cur_liab_due_1y")
            if not df.empty:
                latest = df.sort_values("end_date", ascending=False).iloc[0]
                st_borr = latest.get("st_borr", 0) or 0
                non_cur_due = latest.get("non_cur_liab_due_1y", 0) or 0
                short_debt = format_number(st_borr + non_cur_due)
        except Exception:
            pass

        # Get income statement data for EBIT and interest expense
        ebit = "—"
        interest_expense = "—"
        interest_coverage = "—"
        try:
            df = self._safe_call("income", ts_code=ts_code, report_type="1", fields="ts_code,end_date,operate_profit,finance_exp")
            if not df.empty:
                latest = df.sort_values("end_date", ascending=False).iloc[0]
                ebit = format_number(latest.get("operate_profit", 0))
                interest_expense = format_number(latest.get("finance_exp", 0))
                # Calculate interest coverage ratio
                finance_exp = latest.get("finance_exp", 0) or 0
                operate_profit = latest.get("operate_profit", 0) or 0
                if finance_exp > 0:
                    interest_coverage = f"{operate_profit / finance_exp:.2f}"
        except Exception:
            pass

        rows = [
            ["经营活动现金流 (OCF)", ocf],
            ["短期债务 (含一年内到期非流动负债)", short_debt],
            ["息税前利润 (EBIT)", ebit],
    ...
    def get_key_assumptions(self, ts_code: str) -> str:
        """Section 20: Key assumptions table."""
        lines = [format_header(2, "20. 关键假设表"), ""]

        # Get Capex data
        capex = "—"
        try:
            df = self._safe_call("cashflow", ts_code=ts_code, report_type="1", fields="ts_code,end_date,c_pay_acq_const_fiolta")
            if not df.empty:
                latest = df.sort_values("end_date", ascending=False).iloc[0]
                capex = format_number(latest.get("c_pay_acq_const_fiolta", 0))
        except Exception:
            pass
        ...
        da = "—"
        try:
            df = self._safe_call("cashflow", ts_code=ts_code, report_type="1", fields="ts_code,end_date,depr_fa_coga_dpba,amort_intang_assets,lt_amort_deferred_exp")
            if not df.empty:
                latest = df.sort_values("end_date", ascending=False).iloc[0]
                depr = latest.get("depr_fa_coga_dpba", 0) or 0
                amort_intang = latest.get("amort_intang_assets", 0) or 0
                amort_deferred = latest.get("lt_amort_deferred_exp", 0) or 0
                da = format_number(depr + amort_intang + amort_deferred)
        except Exception:
            pass

        # Get dividend data (implemented + proposed)
        dividend_implemented = "—"
        dividend_proposed = "—"
        try:
            df = self._safe_call("dividend", ts_code=ts_code, fields="ts_code,end_date,div_proc,cash_div")
            if not df.empty:
                # Implemented dividends
                impl_df = df[df["div_proc"] == "实施"].copy()
                if not impl_df.empty:
                    latest_impl = impl_df.sort_values("end_date", ascending=False).iloc[0]
                    dividend_implemented = format_number(latest_impl.get("cash_div", 0))
                # Proposed dividends
                plan_df = df[df["div_proc"].isin(["董事会预案", "股东大会预案"])].copy()
                if not plan_df.empty:
                    latest_plan = plan_df.sort_values("end_date", ascending=False).iloc[0]
                    dividend_proposed = format_number(latest_plan.get("cash_div", 0))
        except Exception:
            pass
        ...
        rows = [
            ["资本支出 (Capex)", capex],
            ["折旧与摊销 (D&A)", da],
            ["分红 (实施)", dividend_implemented],
            ["分红 (预案)", dividend_proposed],
        ]
    ...
    def _get_payout_by_year(self) -> dict[str, float]:
        ...
        if div_raw is None:
            div_any = self._store.get("dividends")
            if div_any is not None and not div_any.empty:
                div_any = div_any.copy()
                if "fiscal_year" not in div_any.columns:
                    div_any["fiscal_year"] = div_any.get("end_date").astype(str).str[:4]
                proc_str = div_any.get("div_proc").astype(str)
                div_raw = div_any[proc_str == "实施"].copy()
                div_plan = div_any[(proc_str.str.contains("预案") | proc_str.str.contains("通过")) & (proc_str != "实施")].copy()
        income_df = self._get_annual_df("income")
    ...
    def _compute_factor4_ev_baseline(self, ts_code: str) -> str | None:
        ...
        # Calculate EV and related metrics
        # EV = 市值 + 有息负债 - 现金
        cash = (cash_yuan / 1e6) if cash_yuan else 0
        ibd = (ibd_yuan / 1e6) if ibd_yuan else 0
        ev = mkt_cap + ibd - cash
        net_debt = ibd - cash
        net_cash = -net_debt
        ...
        # Calculate FCF yield correctly
        fcf_yield = "—"
        if mkt_cap > 0:
            # Use FCFF from financial indicators if available
            if fcf > 0:
                fcf_yield = f"{fcf / mkt_cap * 100:.2f}%"
            # Fallback to OCF - Capex if FCFF not available
            else:
                cf_df = self._store.get("cash_flow")
                if cf_df is not None and not cf_df.empty:
                    latest_cf = cf_df.iloc[0]
                    ocf = self._safe_float(latest_cf.get("n_cashflow_act")) / 1e6 if latest_cf.get("n_cashflow_act") else 0
                    capex = self._safe_float(latest_cf.get("c_pay_acq_const_fiolta")) / 1e6 if latest_cf.get("c_pay_acq_const_fiolta") else 0
                    if ocf > 0:
                        calc_fcf = ocf - abs(capex)
                        if calc_fcf > 0:
                            fcf_yield = f"{calc_fcf / mkt_cap * 100:.2f}%"
        ...
        # Dividend yield: TTM yield (consistent with Section 6)
        div_yield_str = "—"
        ...
        if div_df is not None and not div_df.empty and latest_price > 0:
            # Calculate TTM dividend yield (consistent with Section 6)
            div_df["ex_date_dt"] = pd.to_datetime(div_df["ex_date"], format="%Y%m%d", errors="coerce")
            latest_date = div_df["ex_date_dt"].max()
            if pd.notna(latest_date):
                ttm_start = latest_date - pd.DateOffset(years=1)
                ttm_df = div_df[div_df["ex_date_dt"] >= ttm_start]
                ttm_dps = ttm_df["cash_div_tax"].sum()
                if ttm_dps > 0:
                    div_yield_str = f"{ttm_dps / latest_price * 100:.2f}%"
        
        # Also calculate expected dividend yield if available
        expected_div_yield = "—"
        plan_df = self._store.get("dividends_plan")
        if plan_df is not None and not plan_df.empty and latest_price > 0:
            plan_dps = plan_df["cash_div_tax"].sum()
            if ttm_dps > 0 or plan_dps > 0:
                total_dps = ttm_dps + plan_dps
                expected_div_yield = f"{total_dps / latest_price * 100:.2f}%"
    ...
    def _build_section_13(self, ts_code: str, wc: "WarningsCollector") -> str:
        ...
        try:
            wc.check_industry_profile(self._store.get("industry_profile") or {})
            if self._is_hk(ts_code):
    ...
    def assemble_data_pack(self, ts_code: str) -> str:
        """Assemble complete data_pack_market.md with optimized structure.
        
        New structure (逻辑连贯、无冗余):
        一、公司基本信息
        二、核心业务与经营分析
        三、核心财务报表
        四、关键财务指标与盈利质量
        ...
        company_name = self._get_company_name_cached(ts_code)
        
        lines = [
            format_header(1, f"{company_name}（{ts_code}）分析数据包"),
            "",
            f"*生成时间: {timestamp}*",
            f"*数据来源: Tushare Pro*",
        ...
        # ========== 一、公司基本信息 ==========
        lines.append(format_header(1, "一、公司基本信息"))
        lines.append("")
        total_sections += 1
        try:
            print("  Collecting 1. 基本信息...")
            lines.append(self.get_basic_info(ts_code))
            lines.append("")
            completed += 1
        except Exception as e:
        ...
        # ========== 二、核心业务与经营分析 ==========
        lines.append(format_header(1, "二、核心业务与经营分析"))
        lines.append("")
        
        # 2.1 主营业务构成
        total_sections += 1
        try:
            print("  Collecting 主营业务构成...")
            lines.append(self.get_segments(ts_code))
            lines.append("")
            completed += 1
        except Exception as e:
            lines.append(format_header(2, "2.1 主营业务构成"))
            lines.append(f"\n数据获取失败: {e}\n")

        # ========== 三、核心财务报表 ==========
        lines.append(format_header(1, "三、核心财务报表"))
        lines.append("")
        
        # 3.1 合并利润表
        total_sections += 1
        try:
            print("  Collecting 合并利润表...")
            lines.append(self.get_income(ts_code))
            lines.append("")
            completed += 1
        except Exception as e:
            lines.append(format_header(2, "3.1 合并利润表"))
            lines.append(f"\n数据获取失败: {e}\n")
        ...
        if not self._is_hk(ts_code):
            total_sections += 1
            try:
                print("  Collecting 母公司利润表...")
                lines.append(self.get_income_parent(ts_code))
                lines.append("")
                completed += 1
            except Exception as e:
                lines.append(format_header(2, "3.2 母公司利润表"))
                lines.append(f"\n数据获取失败: {e}\n")

        # 3.3 合并资产负债表
        total_sections += 1
        try:
            print("  Collecting 合并资产负债表...")
            lines.append(self.get_balance_sheet(ts_code))
            lines.append("")
            completed += 1
        except Exception as e:
            lines.append(format_header(2, "3.3 合并资产负债表"))
            lines.append(f"\n数据获取失败: {e}\n")

        # 3.4 母公司资产负债表 (A股 only)
        if not self._is_hk(ts_code):
            total_sections += 1
            try:
                print("  Collecting 母公司资产负债表...")
                lines.append(self.get_balance_sheet_parent(ts_code))
                lines.append("")
                completed += 1
            except Exception as e:
                lines.append(format_header(2, "3.4 母公司资产负债表"))
                lines.append(f"\n数据获取失败: {e}\n")

        # 3.5 现金流量表
        total_sections += 1
        try:
            print("  Collecting 现金流量表...")
            lines.append(self.get_cashflow(ts_code))
            lines.append("")
            completed += 1
        except Exception as e:
            lines.append(format_header(2, "3.5 现金流量表"))
            lines.append(f"\n数据获取失败: {e}\n")

        # ========== 四、关键财务指标与盈利质量 ==========
        lines.append(format_header(1, "四、关键财务指标与盈利质量"))
        lines.append("")
        
        total_sections += 1
        try:
            print("  Collecting 关键财务指标...")
            lines.append(self.get_fina_indicators(ts_code))
            lines.append("")
            completed += 1
        except Exception as e:
            lines.append(format_header(2, "4.1 关键财务指标"))
            lines.append(f"\n数据获取失败: {e}\n")

        # ========== 五、股东、治理与资本运作 ==========
        lines.append(format_header(1, "五、股东、治理与资本运作"))
        lines.append("")
        
        # 5.1 股东与治理
        total_sections += 1
        try:
            print("  Collecting 股东与治理...")
            lines.append(self.get_holders(ts_code))
            lines.append("")
            completed += 1
        except Exception as e:
            lines.append(format_header(2, "5.1 股东与治理"))
            lines.append(f"\n数据获取失败: {e}\n")

        # 5.2 分红历史
        total_sections += 1
        try:
            print("  Collecting 分红历史...")
            lines.append(self.get_dividends(ts_code))
            lines.append("")
            completed += 1
        except Exception as e:
            lines.append(format_header(2, "5.2 分红历史"))
            lines.append(f"\n数据获取失败: {e}\n")
        ...
        if not self._is_hk(ts_code):
            try:
                print("  Collecting 股票回购...")
                lines.append(self.get_repurchase(ts_code))
                lines.append("")
                completed += 1
            except Exception as e:
                lines.append(format_header(2, "5.3 股票回购"))
                lines.append(f"\n数据获取失败: {e}\n")
        else:
            lines.append(format_header(2, "5.3 股票回购"))
            lines.append("\n港股暂不支持\n")
        ...
        if not self._is_hk(ts_code):
            try:
                print("  Collecting 股权质押...")
                lines.append(self.get_pledge_stat(ts_code))
                lines.append("")
                completed += 1
            except Exception as e:
                lines.append(format_header(2, "5.4 股权质押"))
                lines.append(f"\n数据获取失败: {e}\n")
        else:
            lines.append(format_header(2, "5.4 股权质押"))
            lines.append("\n不适用 (港股无此制度)\n")
        ...
        # ========== 六、市场行情与估值分析 ==========
        lines.append(format_header(1, "六、市场行情与估值分析"))
        lines.append("")
        
        # 6.1 市场行情
        total_sections += 1
        try:
            print("  Collecting 市场行情...")
            lines.append(self.get_market_data(ts_code))
            lines.append("")
            completed += 1
        except Exception as e:
            yf_data = self._yf_fallback_price(ts_code)
            if yf_data:
                lines.append(format_header(2, "6.1 市场行情"))
                lines.append(f"\n*来源: yfinance (降级)*")
        ...
        total_sections += 1
        try:
            print("  Collecting 无风险利率...")
            lines.append(self.get_risk_free_rate(ts_code))
            lines.append("")
            completed += 1
        except Exception as e:
            lines.append(format_header(2, "6.2 无风险利率"))
            lines.append(f"\n数据获取失败: {e}\n")

        # ========== 七、风险警示与敏感性分析 ==========
        lines.append(format_header(1, "七、风险警示与敏感性分析"))
        lines.append("")
        
        total_sections += 1
        try:
            print("  Collecting 风险警示...")
            lines.append(self._build_section_13(ts_code, wc))
            lines.append("")
            completed += 1
        except Exception as e:
            lines.append(format_header(2, "7.1 风险警示"))
            lines.append(f"\n数据获取失败: {e}\n")

        # ========== 八、分析用衍生指标与参数 ==========
        lines.append(format_header(1, "八、分析用衍生指标与参数"))
        lines.append("")
        
        total_sections += 1
        try:
            print("  Computing 衍生指标...")
            lines.append(self.compute_derived_metrics(ts_code))
            lines.append("")
            completed += 1
        except Exception as e:
        ...
        lines.append("---")
        lines.append(f"*共 {completed}/{total_sections} 个数据板块成功获取*")

        return "\n".join(lines)

    def _get_company_name_cached(self, ts_code: str) -> str:
        """Get company name from cached basic info."""
        if "basic_info" in self._store:
            return str(self._store["basic_info"].iloc[0].get("name", ts_code))
        return ts_code

    def load_pdf_sections(self, pdf_json_path: str) -> Dict[str, Optional[str]]:
        """Load PDF extracted sections from JSON file.
        
        Args:
            pdf_json_path: Path to pdf_sections.json
        
        Returns:
            Dictionary of section_id -> section_text
        """
        import json
        if not os.path.exists(pdf_json_path):
            print(f"  [WARN] PDF sections file not found: {pdf_json_path}")
            return {}
        ...
        self._store["pdf_sections"] = sections
        return sections

    def integrate_pdf_sections(self, ts_code: str, pdf_sections: Dict[str, Optional[str]]) -> str:
        """Integrate PDF extracted sections into data pack format.
        ...
        Returns:
            Markdown formatted PDF sections
        """
        lines = []
        lines.append(format_header(1, "PDF 财报提取内容"))
        lines.append("")
        lines.append("*以下内容从财报 PDF 直接提取，需与 Tushare 数据交叉验证*")
        lines.append("")
        ...
        section_names = {
            ...
            "MDA_RISK": "风险因素",
            "SUB": "主要子公司",
            "INDUSTRY_EXTRA": "行业适配数据",
        }
        
        critical_sections = ["AUDIT", "BIZ_OVERVIEW", "BIZ", "CAP", "CUS", "DEBT", "MDA_RISK"]
        
        for section_id, section_name in section_names.items():
            text = pdf_sections.get(section_id)
            is_critical = section_id in critical_sections
            
            if text and len(text.strip()) > 50:
                critical_mark = "🔴" if is_critical else "🟢"
                lines.append(format_header(2, f"{critical_mark} {section_name}"))
                lines.append("")
                lines.append(text[:3000] + "..." if len(text) > 3000 else text)
                lines.append("")
            elif is_critical:
                lines.append(format_header(2, f"🔴 {section_name}"))
                lines.append("")
                lines.append("⚠️ **未提取到数据，需人工补充**")
                lines.append("")
        
        return "\n".join(lines)

    def validate_with_pdf(self, ts_code: str, pdf_sections: Dict[str, Optional[str]]) -> Tuple[str, List[str]]:
        """Validate Tushare data against PDF extracted data.
        ...
        Returns:
            Tuple of (validation_report_markdown, missing_critical_list)
        """
        from data_validator import validate_and_annotate
        
        return validate_and_annotate(self._store, pdf_sections, ts_code)

    def assemble_data_pack_with_pdf(
        self, 
        ts_code: str, 
        pdf_sections: Optional[Dict[str, Optional[str]]] = None,
        pdf_json_path: Optional[str] = None,
    ) -> str:
        """Assemble complete data pack with PDF integration and validation.
        
        Args:
            ts_code: Stock code
            pdf_sections: Pre-loaded PDF sections (optional)
            pdf_json_path: Path to pdf_sections.json (optional)
        
        Returns:
            Complete data pack markdown with validation report
        """
        base_pack = self.assemble_data_pack(ts_code)
        
        if pdf_json_path and not pdf_sections:
            pdf_sections = self.load_pdf_sections(pdf_json_path)
        
        if not pdf_sections:
            return base_pack
        
        pdf_content = self.integrate_pdf_sections(ts_code, pdf_sections)
        validation_report, missing_critical = self.validate_with_pdf(ts_code, pdf_sections)
        ...
        if missing_critical:
            lines.append("")
            lines.append("## 🔴 必须补充的关键信息")
            lines.append("")
            lines.append("以下信息对于分析至关重要，但未能从 PDF 或 Tushare 获取：")
            lines.append("")
...
class WarningsCollector:
    ...
    def check_industry_profile(self, industry_profile: dict):
        if not industry_profile:
            return
        if industry_profile.get("needs_manual_confirm"):
            self.warnings.append({
                "type": "INDUSTRY_MAPPING",
                "severity": "中",
                "message": "行业映射缺失或歧义：需人工确认行业类型，相关因子可能降级或关闭",
            })
        if str(industry_profile.get("industry_type", "")).strip() == "金融":
            self.warnings.append({
                "type": "FACTOR_APPLICABILITY",
                "severity": "中",
                "message": "行业类型为金融：因子2-4默认不适用，需使用金融替代框架",
            })
...
Examples:
  ...
  %(prog)s --code 600887.SH --pdf-json output/pdf_sections.json
        """,
    )
    parser.add_argument(
        "--code",
    ...
    parser.add_argument(
        "--pdf-json",
        default=None,
        help="Path to pdf_sections.json for PDF data integration and validation",
    )
...
def main():
    ...
    if args.dry_run:
        ...
        print(f"  PDF JSON: {args.pdf_json or 'none'}")
        return
    ...
    if args.pdf_json and os.path.exists(args.pdf_json):
        print(f"Integrating PDF data from {args.pdf_json}...")
        data_pack = client.assemble_data_pack_with_pdf(ts_code, pdf_json_path=args.pdf_json)
    else:
